...
def lambda_handler(event, context):
    ...
    user_info = {"chat_id": chat_id, "first_name": first_name, "last_name": last_name, "message": text}
    ...
    if "/ask" in text:
        ...
        utils.save_item_to_dynamodb(TELEGRAM_TABLE_NAME, item={"PK": "ASK", **user_info, "response": response})

    elif "/feedback" in text:
        text = text.replace("/feedback", "").strip()
        utils.save_item_to_dynamodb(TELEGRAM_TABLE_NAME, item={"PK": "FEEDBACK", **user_info})
        utils.send_message(chat_id, "Thank you for your feedback!", bot_token)
        utils.send_message(MY_CHAT_ID, f"📝 Feedback from {first_name} {last_name}: \n\n{text}", bot_token)
        ...
        utils.save_item_to_dynamodb(TELEGRAM_TABLE_NAME, item={"PK": "FEATURE", **user_info})
        utils.send_message(MY_CHAT_ID, f"✨ Feature suggestion from {first_name} {last_name}: \n\n{text}", bot_token)

    elif "/bug" in text:
        text = text.replace("/bug", "").strip()
        utils.send_message(chat_id, "Thank you for reporting the bug!", bot_token)
        utils.save_item_to_dynamodb(TELEGRAM_TABLE_NAME, item={"PK": "BUG", **user_info})
        utils.send_message(MY_CHAT_ID, f"🐞 Bug report from {first_name} {last_name}: \n\n{text}", bot_token)
