...
for endpoint in endpoints:
    ...
    if "{" in endpoint["endpoint"] and "}" in endpoint["endpoint"]:
        ...
        if not dataclasses.is_dataclass(path):
            raise Exception(f"Path is not a dataclass on {file_path.replace('.', '/')}")
    ...
    if not dataclasses.is_dataclass(input_):
        raise Exception(f"Input is not a dataclass on {file_path.replace('.', '/')}")

    if not dataclasses.is_dataclass(output):
        raise Exception(f"Output is not a dataclass on {file_path.replace('.', '/')}")
