...
def track(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if function_name == "create_function":
          ...
          full_path = f"{path}/{directory}/main.lambda_handler" if directory else f"{path}/main.lambda_handler"        
          name = kwargs["name"]
          existent_function = next((i for i in data if i["name"] == name), None)
          if not existent_function:
            data.append({ "name": name, "file_path": full_path, "description": kwargs["description"]})
