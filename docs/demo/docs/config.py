...
class DocsConfig:
    def __init__(self, services: Services) -> None:
        ...
        services.api_gateway.create_docs(
            endpoint="/diagram", artifact="diagram", public=True, stages=["Prod"]
        )

        # Tests Report at /tests
        services.api_gateway.create_docs(
            endpoint="/tests", artifact="tests", public=True, stages=["Staging"]
        )

        # Coverage Report at /coverage
        services.api_gateway.create_docs(
            endpoint="/coverage", artifact="coverage", public=True, stages=["Staging"]
        )
