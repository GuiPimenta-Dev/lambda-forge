...
Once deployed, Lambda Forge exposes the following documentation through your project's API Gateway endpoints:
...
> Each document is served from your own API Gateway at routes such as `/swagger`, `/redoc`, `/diagram`, `/coverage`, `/tests` and `/wiki`. The sections below explain how each one is generated.
